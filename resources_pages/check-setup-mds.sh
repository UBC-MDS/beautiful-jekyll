...
if ! [ -x "$(command -v conda)" ]; then  # Check that conda exists as an executable program
    ...
    py_pkgs=(otter-grader=5 pandas=2 nbconvert-core=7 jupyterlab=4 jupyterlab-git=0 jupyterlab-spellchecker=0)
...
if ! [ -x "$(command -v R)" ]; then  # Check that R exists as an executable program
    ...
    if ! Rscript -e "rmarkdown::find_pandoc(dir = c('/usr/lib/rstudio/resources/app/bin/quarto/bin/tools', 'C:/Program Files/RStudio/bin/quarto/bin/tools', '/Applications/RStudio.app/Contents/MacOS/quarto/bin/tools'), cache=F); rmarkdown::render('mds-knit-pdf-test.Rmd', output_format = 'pdf_document')" &> /dev/null; then
